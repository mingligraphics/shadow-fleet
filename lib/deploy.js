...
async function run() {
  ...
  const { env } = response;
  const destination =
    env === 'development' ? `${config.slug}-${config.uuid}-dev` : `${config.slug}-${config.uuid}`;
...
async function attachFallbackImage(destination) {
  ...
  const dimensions = imageSize(image.body);
...
async function upload(destination) {
  ...
  try {
    ...
    const insetResponse = responses.find(response => /\.json$/.test(response.body.urls.cached));
